...
fn index() -> Html<&'static str> {
    ...
    <script>
        ...
        loginForm.addEventListener("submit", function(event) {
            ...
            msgInput.focus();
            let secure = window.location.protocol.includes('s') ? 's' : '';
            ws = new WebSocket(`ws${secure}://${window.location.host}/ws/` + nameInput.value);
